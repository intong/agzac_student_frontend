  ...
  const makeApiArr = {
    ...
    getDevelopmentReason: () => {
      ...
      if (firstTxtArea !== undefined) {
        const mark2Break1 = firstTxtArea.replace(/\[.*?\]/g, ""); // 대괄호제거
        const markBreak1 = mark2Break1.replace('"', ""); // 따옴표 제거
        const lineBreak1 = markBreak1.replace(/(\r\n\t|\n|\r\t)/gm, ""); // 줄바꿈 제거
        const spaceBreak1 = lineBreak1.split(" ").join(""); // space 공백 띄어쓰기 제거
        if (spaceBreak1.length > 4) {
          tempArr.push(spaceBreak1);
          if (secondTxtArea !== undefined) {
            const mark2Break2 = secondTxtArea.replace(/\[.*?\]/g, ""); // 대괄호제거
            const markBreak2 = mark2Break2.replace('"', ""); // 따옴표 제거
            const lineBreak2 = markBreak2.replace(/(\r\n\t|\n|\r\t)/gm, ""); // 줄바꿈 제거
            const spaceBreak2 = lineBreak2.split(" ").join(""); // space 공백 띄어쓰기 제거
            if (spaceBreak2.length > 4) {
              tempArr.push(spaceBreak2);
              if (thirdTxtArea !== undefined) {
                const mark2Break3 = thirdTxtArea.replace(/\[.*?\]/g, ""); // 대괄호제거
                const markBreak3 = mark2Break3.replace('"', ""); // 따옴표 제거
                const lineBreak3 = markBreak3.replace(/(\r\n\t|\n|\r\t)/gm, ""); // 줄바꿈 제거
                const spaceBreak3 = lineBreak3.split(" ").join(""); // space 공백 띄어쓰기 제거
                if (spaceBreak3.length > 9) {
                  tempArr.push(spaceBreak3);
                  if (fourthTxtArea !== undefined) {
                    const mark2Break4 = fourthTxtArea.replace(/\[.*?\]/g, ""); // 대괄호제거
                    const markBreak4 = mark2Break4.replace('"', ""); // 따옴표 제거
                    const lineBreak4 = markBreak4.replace(
                      /(\r\n\t|\n|\r\t)/gm,
                      ""
                    ); // 줄바꿈 제거
                    const spaceBreak4 = lineBreak4.split(" ").join(""); // space 공백 띄어쓰기 제거
                    if (spaceBreak4.length > 9) {
                      tempArr.push(spaceBreak4);
                      return { ok: true, data: tempArr };
                    } else {
                      alert("10자 이상 작성해주세요.");
                      return { ok: false, data: tempArr };
    ...
    getFutureHumanRole: () => {
      ...
      if (inner1 !== "선택") {
        ...
        if (humanRole1 !== undefined) {
          const mark2Break1 = humanRole1.replace(/\[.*?\]/g, ""); // 대괄호제거
          const markBreak1 = mark2Break1.replace('"', ""); // 따옴표 제거
          const lineBreak1 = markBreak1.replace(/(\r\n\t|\n|\r\t)/gm, ""); // 줄바꿈 제거
          const spaceBreak1 = lineBreak1.split(" ").join(""); // space 공백 띄어쓰기 제거
          if (spaceBreak1.length > 9) {
            tempArr.push(spaceBreak1);
            if (inner2 !== "선택") {
              checkOverlap.push(inner2);
              if (humanRole2 !== undefined) {
                const mark2Break2 = humanRole2.replace(/\[.*?\]/g, ""); // 대괄호제거
                const markBreak2 = mark2Break2.replace('"', ""); // 따옴표 제거
                const lineBreak2 = markBreak2.replace(/(\r\n\t|\n|\r\t)/gm, ""); // 줄바꿈 제거
                const spaceBreak2 = lineBreak2.split(" ").join(""); // space 공백 띄어쓰기 제거
                if (spaceBreak2.length > 9) {
                  tempArr.push(spaceBreak2);
                  if (inner3 !== "선택") {
                    checkOverlap.push(inner3);
                    if (humanRole3 !== undefined) {
                      const mark2Break3 = humanRole3.replace(/\[.*?\]/g, ""); // 대괄호제거
                      const markBreak3 = mark2Break3.replace('"', ""); // 따옴표 제거
                      const lineBreak3 = markBreak3.replace(
                        /(\r\n\t|\n|\r\t)/gm,
                        ""
    ...
    getItemItro: () => {
      ...
      if (itemItro1 !== undefined) {
        const mark2Break1 = itemItro1.replace(/\[.*?\]/g, ""); // 대괄호제거
        const markBreak1 = mark2Break1.replace('"', ""); // 따옴표 제거
        const lineBreak1 = markBreak1.replace(/(\r\n\t|\n|\r\t)/gm, ""); // 줄바꿈 제거
        const spaceBreak1 = lineBreak1.split(" ").join(""); // space 공백 띄어쓰기 제거
        if (spaceBreak1.length > 9) {
          ...
          if (itemItro2 !== undefined) {
            const mark2Break2 = itemItro2.replace(/\[.*?\]/g, ""); // 대괄호제거
            const markBreak2 = mark2Break2.replace('"', ""); // 따옴표 제거
            const lineBreak2 = markBreak2.replace(/(\r\n\t|\n|\r\t)/gm, ""); // 줄바꿈 제거
            const spaceBreak2 = lineBreak2.split(" ").join(""); // space 공백 띄어쓰기 제거
            if (spaceBreak2.length > 14) {
              tempArr.push(spaceBreak2);
              if (itemItro3 !== undefined) {
                const mark2Break3 = itemItro3.replace(/\[.*?\]/g, ""); // 대괄호제거
                const markBreak3 = mark2Break3.replace('"', ""); // 따옴표 제거
                const lineBreak3 = markBreak3.replace(/(\r\n\t|\n|\r\t)/gm, ""); // 줄바꿈 제거
                const spaceBreak3 = lineBreak3.split(" ").join(""); // space 공백 띄어쓰기 제거
                if (spaceBreak3.length > 14) {
                  tempArr.push(spaceBreak3);
                  if (itemItro4 !== undefined) {
                    const mark2Break4 = itemItro4.replace(/\[.*?\]/g, ""); // 대괄호제거
                    const markBreak4 = mark2Break4.replace('"', ""); // 따옴표 제거
                    const lineBreak4 = markBreak4.replace(
                      /(\r\n\t|\n|\r\t)/gm,
                      ""
